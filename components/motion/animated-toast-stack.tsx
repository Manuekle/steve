...
const ToastItem = memo(function ToastItem({
  ...
  return (
    ...
    >
      <div
        className={cn(
          "relative overflow-hidden rounded-2xl border border-border bg-card p-3 shadow-[var(--shadow-elevated),var(--shadow-inset)]",
          classNames?.surface,
        )}
      >
        {renderToast ? (
          ...
          <div className="flex items-start gap-3">
            <motion.span
              layout
              className={cn(
                "mt-0.5 inline-flex h-7 w-7 shrink-0 items-center justify-center rounded-lg shadow-[var(--shadow-inset)]",
                STATUS_CLASS[status],
                classNames?.iconWrap,
              )}
            >
            ...
            <div className={cn("min-w-0 flex-1", classNames?.content)}>
              ...
              {toast.action ? (
                <button
                  ...
                  className={cn(
                    "mt-2 inline-flex h-7 items-center rounded-[9px] border border-border bg-background px-3 text-xs font-medium text-foreground shadow-[var(--shadow-button)] transition-colors hover:bg-muted",
                    classNames?.action,
                  )}
                >
                  {toast.action.label}
            ...
            {canDismiss ? (
              <button
                ...
                className={cn(
                  "inline-flex h-7 w-7 shrink-0 items-center justify-center rounded-[9px] text-muted-foreground transition-colors hover:bg-muted hover:text-foreground",
                  classNames?.close,
                )}
              >
