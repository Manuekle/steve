...
const VT_CSS = `
/* The app cross-fades :root's background and colour on every theme change.
   During a view transition that fade runs *under* the reveal and turns it to
   mush, so it is suspended for the length of the transition. */
html[data-beui-vt] {
...
export function useThemeToggle({
  ...
  /** `toggle` takes an optional origin ("x% y%") that wins over `start`. */
  ...
  const toggle = (origin?: string) => {
    ...
    if (variant === "rectangle") {
      ...
      root.style.setProperty("--beui-vt-origin", origin ?? CIRCLE_ORIGIN[start]);
    ...
    // A transition can abort — a second toggle mid-reveal, or a tab that
    // isn't visible — and `finished` rejects when it does. That rejection is
    // nothing to act on, but unhandled it lands in the console as an
    // InvalidStateError, so it's swallowed here after the cleanup.
    vt.finished
      .finally(() => {
