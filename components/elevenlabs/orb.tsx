...
/* ElevenLabs UI registry component (ui.elevenlabs.io/r/orb.json), vendored.
 *
 * These live under `components/elevenlabs/` rather than `components/ui/`
 * because the registry ships its own `orb.tsx`, and `components/ui/orb.tsx` is
 * already the app's `thinking-orbs` orb used in five places. Keeping the
 * registry's files in their own folder means an upgrade never silently
 * overwrites an app component.
 *
 * One edit: the perlin-noise texture is served from `public/elevenlabs/`
 * instead of the upstream Google CDN URL. A self-hosted install should not
 * need a third-party CDN reachable to render its own screen.
 */
  ...
  return (
    <div className={className ?? "relative h-full w-full"}>
      <Canvas
        ...
        onCreated={({ gl }) => {
          // Ensure canvas is opaque where needed - prevents flash white
...
function Scene({
  ...
  useFrame((_, delta: number) => {
    ...
    // Always opaque - starting at 1 avoids blank flash while texture suspends
    u.uOpacity.value = 1
  ...
  const uniforms = useMemo(() => {
    ...
    // Ensure texture is ready - prevents white flash on first frame
