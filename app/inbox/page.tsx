...
export default function InboxPage() {
  ...
  const [newPhoneCountry, setNewPhoneCountry] = useState("+54");
  const [phoneCountries, setPhoneCountries] = useState<Array<{ code: string; name: string }>>([]);
  ...
  useEffect(() => {
    fetch("https://api.restcountries.com/v3.1/all?fields=name,idd,cca2")
      ...
      .then(function (data) {
        ...
        if (list.length > 0) setPhoneCountries(list);
      })
      .catch(() => null);
  ...
  const validateCreateForm = useCallback((): boolean => {
    ...
    const fullPhoneCheck = newPhone.trim() ? (newPhone.trim().startsWith("+") ? newPhone.trim() : `${newPhoneCountry}${newPhone.trim().replace(/^0+/, "")}`) : "";
    ...
    return !hasError;
  }, [newName, newPhone, newPhoneCountry, newEmail, triggerShake, t]);
  ...
  const createContact = (e: FormEvent) => {
    ...
    const fullPhone = newPhone.trim() ? (newPhone.trim().startsWith("+") ? newPhone.trim() : `${newPhoneCountry}${newPhone.trim().replace(/^0+/, "")}`) : undefined;
    void send({
      method: "POST",
      headers: { "content-type": "application/json" },
      body: JSON.stringify({
                      ...
                      <div className={cn("t-input-wrap", formErrors.phone && "is-error")}>
                        <label className="block space-y-1.5 text-sm">
                          ...
                          <div className="flex gap-2">
                            <Select onValueChange={setNewPhoneCountry} value={newPhoneCountry}>
                    ...
                    <Card key={contact.id}>
                      <div className="flex items-center gap-3 px-5 py-4">
                        ...
                        <div className="flex size-10 shrink-0 items-center justify-center rounded-xl bg-muted text-muted-foreground shadow-[var(--shadow-inset)]">
                          <ChannelIcon channel={contact.channel === "form" ? "web" : contact.channel} />
                        </div>
                        <div className="min-w-0 flex-1">
                          <div className="flex items-center gap-2">
                            <p className="truncate text-sm font-medium">{contact.name}</p>
                            <span className="rounded-md bg-muted px-1.5 py-0.5 text-[10px] font-medium uppercase tracking-wide text-muted-foreground">
                              {contact.status === "followup_due"
                                ? t("inbox.followup")
                                : t("inbox.handoff")}
                            </span>
                          </div>
