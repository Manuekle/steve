...
#   ./scripts/install-backup-agent.sh --offsite /Volumes/Backups/senka
#   ./scripts/install-backup-agent.sh --offsite-command ~/bin/senka-offsite
  ...
  case "$1" in
    ...
    --offsite)
      ...
      if [ "$2" = "icloud" ]; then
        ...
        OFFSITE_DIR="$ICLOUD/senka-backups"
      else
        OFFSITE_DIR="$2"
      fi
      shift 2 ;;
...
case "$ACTION" in
  status)
    ...
    echo
    [ -f "$HOME/senka-backups/backup.log" ] && tail -20 "$HOME/senka-backups/backup.log" || echo "(no log yet)"
    exit 0 ;;
  run)
    launchctl kickstart -p "gui/$(id -u)/$LABEL"
    echo "started; watch: tail -f ~/senka-backups/backup.log"
    exit 0 ;;
  uninstall)
    launchctl bootout "gui/$(id -u)/$LABEL" 2>/dev/null || true
    rm -f "$PLIST"
    echo "removed. Dumps in ~/senka-backups were left alone."
    exit 0 ;;
esac
...
mkdir -p "$HOME/Library/LaunchAgents" "$HOME/senka-backups"
chmod 700 "$HOME/senka-backups"
...
echo "  plist:  $PLIST"
echo "  dumps:  $HOME/senka-backups"
echo "  log:    $HOME/senka-backups/backup.log"
